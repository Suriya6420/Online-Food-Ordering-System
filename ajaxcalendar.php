...
self.location='ajaxcalendar.php?month=' + month_val + '&year=' + year_val ; // reload the page
}
</script>
<style type="text/css">
table.main {
...
echo "<select name=month value='' id='month' class='form-control'>
...
echo "</select>";
echo "</td><td>";
echo "<select name=year value='' id='year' class='form-control'>
...
echo "</td><td>";
?>
<input type='button' name='btnselect' class='form-control' value='Select Calendar' style='width: 200px;' onclick="loadcalendar(month.value,year.value);">
<?php
echo "</td></tr>";
...
$pv="'$month'".","."'$i'".","."'$year'";
$tim=date("H:i A");
echo $adj."<td style='height: 50px;'><center><b>$i</b><br>
<input type='checkbox' id='selecteddt[$i]' name='selecteddt[]' value='$year-$month-$i' onchange='insertpayment(this.value,grandtotal.value,$i)' >
<input type='time' name='selectedtime[]'  onchange='insertpaymenttime(`$year-$month-$i`,grandtotal.value,$i,this.value)' >
</center></td>";
$adj='';
$j ++;
	if($j==7)
