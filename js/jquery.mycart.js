...
(function ($) {
  ...
  var loadMyCartEvent = function(userOptions){
    ...
    if(!$("#" + idCartModal).length) {
      $('body').append(
        '<form method="post" action="order.php" ><div class="modal fade" id="' + idCartModal + '" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">' +
    ...
    var drawTable = function(){
      ...
      $.each(products, function(){
        ...
        $cartTable.append(
          ...
          '<td>' + this.name + '</td>' +
          '<td title="Unit Price"> ₹' + this.price + '<input type="hidden" name="id[]" value=' + this.id + '><input type="hidden" name="price[]" value=' + this.price + '>' + '</td>' +
          '<td title="Quantity"><input name="quantity[]" type="number" min="1" style="width: 70px;" class="' + classProductQuantity + ' form-control" value="' + this.quantity + '"/><textarea name="description[]" class="form-control" placeholder="Any Note"></textarea></td>' +
          '<td title="Total" class="' + classProductTotal + '"> ₹' + total + '</td>' +
          '<td title="Remove from Cart" class="text-center" style="width: 30px;"><a href="javascript:void(0);" class="btn btn-xs btn-danger ' + classProductRemove + '">X</a></td>' +
          '</tr>'
        );
      });
//<Td><textarea name="description[]" class="form-control" placeholder="Any Note"></textarea></td>
...
/*
      ...
      if(products.length && discountPrice !== null) {
        $cartTable.append(
          '<tr style="color: red">' +
          '<td></td>' +
          '<td><strong>Offer code - <input type="text" id="offercode" name="offercode"  onkeyup="checkoffercode(this.value,' + ProductManager.getTotalPrice() + ')"></strong></td>' +
          '<td id="discounttype"></td>' +
          '<td id="discountamt"></td>' +
...
function checkoffercode(offercode,totalprice)
{
	$.post("ajaxoffer.php", {offercode: offercode,totalprice: totalprice}, function(result)
	{
		alert(result);
		//$("#tableid").html(result.attr1);
        //$("#foot").html(result.attr2);
		//$("#discounttype").html(result.offertype);
		//$("#discountamt").html(result.offeramt);
		//alert($("#offertype").html(result.offertype));
        //alert($("#offertitle").html(result.offertitle));
		//alert($("#offercode").html(result.offercode));
		//alert($("#offeramt").html(result.offeramt));		
	});
}